...
#
# Description:
#   Removes Webroot SecureAnywhere by force. Run in Safe Mode for best results.
#
# Metadata:
...
.DESCRIPTION
    Removes Webroot SecureAnywhere by stopping services, removing registry keys, deleting folders,
    and removing startup entries. Run the script once in Safe Mode, then reboot for complete removal.
.OUTPUTS
    Informational messages written to the host indicating each removal step.
.NOTES
