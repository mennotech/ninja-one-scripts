...
#
# Description:
#   Completely removes the NinjaOne RMM Agent and NinjaOne Remote from the system.
#
# Metadata:
...
.SYNOPSIS
    Completely removes the NinjaOne RMM Agent and NinjaOne Remote from the system.
.DESCRIPTION
    Uninstalls the NinjaOne RMM Agent by running the native uninstaller, stopping services,
    removing processes, cleaning registry keys, and removing installation directories.
    Also removes NinjaOne Remote (ncstreamer), its virtual display driver, printer, and all associated registry entries.
    A transcript log is written to %WINDIR%\temp\.
.OUTPUTS
    Progress and status messages written to the host. A transcript log is also created.
.NOTES
