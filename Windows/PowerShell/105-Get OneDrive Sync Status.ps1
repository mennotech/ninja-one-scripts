...
.PARAMETER RMMOneDriveStateField
    Specifies the RMM field to populate with the Maximum State Value found.
.PARAMETER RMMOneDriveDetailsField
    Specifies the RMM field to populate with the details of OneDrive sync status.
.PARAMETER UpdateScheduledTask
    Specifies whether to update the scheduled task with the new action. Default is "false".
.OUTPUTS
    Outputs the status of the script. If any OneDrive status is above 0 then it will also print out the Sync Details.
.NOTES
    2025-06-24: Initial version of the script.
.LINK
    https://github.com/mennotech/ninja-one-scripts/blob/main/Windows/PowerShell/105-Get%20OneDrive%20Sync%20Status.ps1
.LICENSE
