...
#
# Description:
#   Downloads and installs the CrowdStrike Falcon Sensor with a specified CID.
#
# Metadata:
...
<#
.SYNOPSIS
    Downloads and installs the CrowdStrike Falcon Sensor.
.DESCRIPTION
    Downloads the CrowdStrike Falcon Sensor installer from a pre-configured location
    and installs it silently with the specified CID (Customer ID).
    The CID must be provided via the script variable 'cid' in NinjaOne or via the $env:cid environment variable.
.PARAMETER CustomCID
    The CrowdStrike Customer ID (CID) to use for installation. Overridden by $env:cid if set.
.OUTPUTS
...
begin {
    if ($env:cid -and $env:cid -notlike "null") { $CustomCID = $env:cid }
...
process {
    ...
    Write-Host "Successfully installed and set up Falcon Sensor with CID $($env:cid)"
    exit 0
}
end {
